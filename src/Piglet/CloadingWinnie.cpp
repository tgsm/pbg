...
CLoadingWinnie::CLoadingWinnie(CGame* game) : CLoadingCallback(game) {

}

CLoadingWinnie::~CLoadingWinnie() {
    Destroy();
}

void CLoadingWinnie::Create() {
    CLoadingCallback::Create();

    if (m_game != NULL && m_scene != NULL && m_camera != NULL && m_light != NULL && m_clump24 != NULL && m_light2 != NULL) {
        if (m_clump1C == NULL) {
            BOOL failed = (m_clump1C = m_scene->CloneClump("Models/110_WINNIE/CAR_110.dff", NULL)) == NULL;
            if (failed) {
                return;
            }
        }
...
void CLoadingWinnie::Destroy() {
    CLoadingCallback::Destroy();
}

void CLoadingWinnie::Update() {
