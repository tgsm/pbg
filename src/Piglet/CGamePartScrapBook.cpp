...
CGamePartScrapBook::CGamePartScrapBook(CGame* game) {
    ...
    m_game->LoadConfigFile("MENUS/SCRAPBOOK/SCRAPBOOK.XMD");
...
CGamePartScrapBook::~CGamePartScrapBook() {
...
void CGamePartScrapBook::Render(F32 dt) {
    m_game->GetScene()->SelectCamera(m_game->GetCamera());
...
void CGamePartScrapBook::RenderBackGround(F32 dt) {
...
BOOL CGamePartScrapBook::IsVideoOpen(int index) {
    ...
    if (m_game->GetMission(index).m_unk2C != 0) {
        return TRUE;
    }
...
void CGamePartScrapBook::RenderIcons(F32 dt) {
    CIcon::BeginRender(m_game->m_display_engine, m_game->GetCamera(), m_game->GetScene());
    CIcon icon;
    ...
    for (int i = 0; i < 6; i++) {
        ...
        icon.m_texture = m_game->m_texture_dictionary->FindTexture(name);
        icon.Render(m_batches[i], 1);
    }
    m_game->GetScene()->Flush();
    CIcon::EndRender();
}

U32 CGamePartScrapBook::NextFrameExit() {
